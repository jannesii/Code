...
def novpn_settings():
    ...
    Root-Admin only.
    """
    guard = require_root_admin_or_redirect("You do not have permission to change NoVPN settings.", 'web.get_settings_page')
    if guard:
        return guard
    try:
        if request.method == 'POST':
            if 'add_device' in request.form:
                ...
                logger.debug("Editing device: orig_mac=%s, new_name=%s, new_mac=%s", mac_orig, name, mac_new)
                try:
                    if not mac_orig:
                    ...
                    ok, updated = novpn_update_device_meta(mac_orig, name=name or None, new_mac=mac_new or None)
                    if not ok:
                        # Fallback: if not found, treat as add (use provided fields)
                        nm = name or mac_new
                        mm = mac_new or mac_orig
                        if not nm or not mm:
                            raise ValueError('Please fill both name and MAC when creating a new device.')
                ...
                for l in leases:
                    ...
                    if mac_l and mac_l not in existing_by_mac:
                        name_l = (l.get('hostname') or l.get('ip') or l.get('mac') or '').strip()
                ...
                for d in merged:
                    ...
                    if mac in existing_by_mac:
                        ...
                        try:
                            ok, _ = novpn_add_device(name=str(d.get('name') or mac), mac=mac, novpn=novpn_val, nodns=nodns_val)
                        except Exception as e:
                            ok = False
                    if not ok:
                        errors.append(mac)
        ...
        for l in static_leases_all:
            ...
            if not mac_l or mac_l in existing_macs:
                continue
            name_l = (l.get('hostname') or l.get('ip') or l.get('mac') or '').strip()
            extra_rows.append({'name': name_l, 'mac': mac_l, 'novpn': False, 'nodns': True, 'extra': True})
